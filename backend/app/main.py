...
from app.models.judo import AnalyzeResponse, BracketInput
...
from app.services.simulation_engine import simulate_probabilities
...
async def get_competition(competition_id: int):
    data = await service.get_competition(competition_id)
    return data
...
async def analyze_bracket(payload: BracketInput):
    ...
    strength = {normalize_name(k): v for k, v in payload.manual_strength.items()}
    athletes = {a for matches in bracket.values() for pair in matches for a in pair if a}
    for athlete in athletes:
        strength.setdefault(athlete, 1.0)

    winner_probs, top4_probs = simulate_probabilities(bracket, strength)
